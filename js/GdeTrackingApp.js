...
//				console.log(resp);
				...
				{
//					var emailAddressStartPosition = $scope.userEmails.indexOf("@");
					var emailDomain = $scope.userEmails[i].value.substring($scope.userEmails[i].value.indexOf('@'));
					if ($scope.userEmails[i].value == 'maiera@google.com ')
					{
						console.log('Welcome Googler '+$scope.userName+'! You are an administrator of this app.');
						$('#generalStatisticsForGooglers').css('display','block');
					} else if (emailDomain == '@google.com')
					{
					...
					} else
					{
						...
						.success(function(response, status, config)
						{
							...
							console.log('Welcome GDE '+$scope.userName+'!');
